...
//         echo "</pre>";
    ...
    public function mysort()
    {
        ...
        {
            if($num->isValid($key)){
                $ctprow = $ctp->getWhere($item);
                $subarr = array("mactp"=>$item,'sometvai'=>$ctprow['SoMetVai']);
                $sometvai[$key] = $ctprow['SoMetVai'];
                $opsort[] = $subarr;
            }
        }
        array_multisort($sometvai, SORT_ASC, $opsort);
        
        $myarr= array();
        foreach ($opsort as $key=>$item)
        {
            $myarr[] = $item['mactp'];
        }
        $myarr['madonhang'] = $arr['madonhang'];
        unset($mysession->checktp);
        $mysession->checktp = $myarr;     
